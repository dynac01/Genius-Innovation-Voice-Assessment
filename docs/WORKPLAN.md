...
- [ ] Initialise git repo, monorepo tooling (pnpm workspaces), root `tsconfig`, linting, formatting
- [ ] Package skeleton — separating the reusable core from its drivers:
  - [ ] `packages/core` — loop, interfaces, protocol, state machines. **Zero I/O dependencies.**
  - [ ] `packages/providers` — real + fake STT / LLM / TTS implementations
  - [ ] `apps/server` — transport, session management
  - [ ] `apps/web` — browser demo
- [ ] Test runner wired up (Vitest) with a passing smoke test
- [ ] `.devcontainer/devcontainer.json` — Node version, pnpm, port forwarding, post-create install
- [ ] Verify the repo actually opens and runs in a real GitHub Codespace *(do not defer this)*
- [ ] GitHub Actions CI — install, typecheck, lint, test. **No secrets.**
- [ ] `.env.example` documenting every key, with the app defaulting to fakes when unset
...
- [ ] **Test:** fake STT emits partial → short gap → more speech; assert the assistant waits **(criterion 4)**
- [ ] **Test:** TTS receives its first chunk before the LLM stream completes **(criterion 5)**
- [ ] **Test:** transcript updates arrive incrementally, not in one batch at the end **(criterion 5)**
...
- [ ] **Test:** an `interrupt` from the bridge stops TTS emission on the next chunk **(criterion 1)**
- [ ] Instrument and **measure** actual barge-in latency; record the number
...
- [ ] **Test:** `barge_in: pause` followed by a resume continues the remaining text **(criterion 2)**
- [ ] **Test:** new substantive speech abandons the prior reply and it does not resume **(criterion 3)**
...
- [ ] **Test:** earcon events are emitted at the right points in the loop **(criterion 6)**
...
- [ ] Confirm `git diff` shows **zero changes** to `packages/core` across the swap **(criterion 7)**
...
- [ ] **Tests** for all three awkward cases **(criterion 8)**
